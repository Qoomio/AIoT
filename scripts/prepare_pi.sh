...
# ===================================
# STEP 9: Bundle Qoom Code for SD Card
# ===================================
echo ""
if [ "${ROOT_MOUNT_FAILED:-false}" = "true" ]; then
    echo -e "${YELLOW}Step 9: Skipping Qoom bundling (root partition not mounted)${NC}"
    echo "Qoom will need to be installed manually on first boot."
    BUNDLE_COPIED=false
else
    echo -e "${GREEN}Step 9: Bundling Qoom code for SD card...${NC}"

    # Get the qoom project root (4 levels up from setup/prepare_pi.sh)
    # Path: products/kits/base_aiot/setup -> products/kits/base_aiot -> products/kits -> products -> qoom2
QOOM_PROJECT_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

if [ ! -f "$QOOM_PROJECT_DIR/package.json" ]; then
    echo -e "${RED}Error: Could not find qoom project at $QOOM_PROJECT_DIR${NC}"
    echo "Make sure you're running this script from the qoom repository."
    exit 1
fi

# Use student.pi.json to determine what to bundle
STUDENT_CONFIG="$QOOM_PROJECT_DIR/services/student.pi.json"
if [ ! -f "$STUDENT_CONFIG" ]; then
    echo -e "${RED}Error: Could not find student.pi.json at $STUDENT_CONFIG${NC}"
    exit 1
fi

echo "Building student Pi bundle from: $STUDENT_CONFIG"

# Create staging directory
STAGING_DIR="/tmp/qoom-staging-$$"
# Only create applets and logs directories - others will be copied from "other" files
mkdir -p "$STAGING_DIR/qoom/applets"
mkdir -p "$STAGING_DIR/qoom/logs"

# Copy applets specified in student.pi.json using Python for proper JSON parsing
echo "Copying applets..."
python3 -c "
import json
with open('$STUDENT_CONFIG') as f:
    config = json.load(f)
for applet in config.get('applets', []):
    print(applet)
" | while read applet; do
    if [ -d "$QOOM_PROJECT_DIR/applets/$applet" ]; then
        echo "  - $applet"
        cp -r "$QOOM_PROJECT_DIR/applets/$applet" "$STAGING_DIR/qoom/applets/"
    else
        echo "  - $applet (not found, skipping)"
    fi
done

# Copy other files specified in student.pi.json
echo "Copying other files..."
python3 -c "
import json
with open('$STUDENT_CONFIG') as f:
    config = json.load(f)
for item in config.get('other', []):
    print(item)
" | while read item; do
    if [ -e "$QOOM_PROJECT_DIR/$item" ]; then
        echo "  - $item"
        # Create parent directory if needed
        mkdir -p "$STAGING_DIR/qoom/$(dirname "$item")"
        cp -r "$QOOM_PROJECT_DIR/$item" "$STAGING_DIR/qoom/$item"
    else
        echo "  - $item (not found, skipping)"
    fi
done

# Generate package.json from student.pi.json
echo "Generating package.json..."
cat "$STUDENT_CONFIG" | python3 -c "
import json, sys
config = json.load(sys.stdin)
print(json.dumps(config['package'], indent=2))
" > "$STAGING_DIR/qoom/package.json"

# Generate ecosystem.config.cjs from student.pi.json
...
print('  apps: [')
print('    ' + json.dumps(eco, indent=6).replace('\n', '\n    '))
print('  ]')
print('};')
" > "$STAGING_DIR/qoom/ecosystem.config.cjs"

# Remove large unnecessary files from the staged applets
...
# NOTE: Keep monaco-editor and dist - they contain essential editor files
find "$STAGING_DIR" -name "*.log" -delete 2>/dev/null || true
find "$STAGING_DIR" -name "*.pdf" -delete 2>/dev/null || true

# Add pre-installed projects
echo "Adding pre-installed projects..."
mkdir -p "$STAGING_DIR/qoom/projects"

# Add AIoT project from template
AIOT_TEMPLATE="$QOOM_PROJECT_DIR/applets/projecter/templates/aiot"
if [ -d "$AIOT_TEMPLATE" ]; then
    echo "  - AIoT (AIoT project template)"
    mkdir -p "$STAGING_DIR/qoom/projects/AIoT"
    cp -r "$AIOT_TEMPLATE/." "$STAGING_DIR/qoom/projects/AIoT/"
else
    echo "  - AIoT template (not found, skipping)"
fi

# Create the tarball
QOOM_TARBALL="/tmp/qoom-bundle-$$.tar.gz"
echo "Creating bundle..."
tar -czf "$QOOM_TARBALL" -C "$STAGING_DIR" qoom

# Cleanup staging directory
rm -rf "$STAGING_DIR"

BUNDLE_SIZE=$(du -h "$QOOM_TARBALL" | cut -f1)
echo "✓ Bundle created: $BUNDLE_SIZE"

# List what's in the bundle
echo ""
echo "Bundle contents:"
tar -tzf "$QOOM_TARBALL" | grep -E "^qoom/[^/]+$|^qoom/applets/[^/]+/$" | sort | head -20

# Check available space on SD card
echo ""
AVAILABLE_SPACE=$(df -h "$ROOT_MOUNT" | tail -1 | awk '{print $4}')
echo "Available space on SD card: $AVAILABLE_SPACE"

# Copy the bundle to the SD card
echo "Copying bundle to SD card..."
if ! cp "$QOOM_TARBALL" "$ROOT_MOUNT/home/qoom-bundle.tar.gz"; then
    echo -e "${RED}Error: Not enough space on SD card for the bundle${NC}"
    echo "Bundle size: $BUNDLE_SIZE"
    echo "Available space: $AVAILABLE_SPACE"
    echo ""
    echo "Options:"
    echo "  1. Use a larger SD card"
    echo "  2. The Pi will need to clone the repo manually after first boot"
    echo ""
    rm -f "$QOOM_TARBALL"
    
    # Continue without the bundle - the first-boot script has a fallback
    echo -e "${YELLOW}Continuing without bundle - Pi will attempt to clone from git on first boot${NC}"
    BUNDLE_COPIED=false
else
    rm "$QOOM_TARBALL"
    echo "✓ Bundle copied to SD card"
    BUNDLE_COPIED=true
fi

fi  # End of ROOT_MOUNT_FAILED check for Step 9
...
# Step 4: Extract Qoom bundle
echo ""
echo "Step 4: Extracting Qoom application..."
REPO_DIR="$SETUP_HOME/qoom"
BUNDLE_PATH="/home/qoom-bundle.tar.gz"

if [ -f "$BUNDLE_PATH" ]; then
    echo "Found qoom bundle, extracting..."
    ...
    # The tarball contains a directory named after the source (likely qoom2), rename it
    EXTRACTED_DIR=$(ls -d */ 2>/dev/null | grep -E "qoom" | head -1 | tr -d '/')
    if [ -n "$EXTRACTED_DIR" ] && [ "$EXTRACTED_DIR" != "qoom" ]; then
        mv "$EXTRACTED_DIR" qoom
    fi
    ...
    echo "✓ Qoom application extracted"
else
    echo "Warning: Qoom bundle not found at $BUNDLE_PATH"
    echo "The Qoom application will need to be set up manually."
    echo "You can clone the repository after first boot if needed."
fi
...
if [ -d "$REPO_DIR" ]; then
    ...
    echo "Warning: Qoom directory not found, skipping npm install and PM2 start"
fi

# Step 7b: Setup Python/AIoT projects with system-site-packages
echo ""
echo "Step 7b: Setting up Python/AIoT projects..."
...
echo "  ✓ Git installed"
echo "  ✓ Qoom application installed: $REPO_DIR"
echo ""
...
echo "  3. Wait 5-10 minutes for setup to complete (npm install takes time)"
